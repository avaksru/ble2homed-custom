...
async function onDiscovery(peripheral) {
  ...
  if (entered) {
    ...
    if (config.retain) {
      mqtt.send(dev.presence_topic, '{"status":"online"}', '{ retain: true }');
    } else {
      mqtt.send(dev.presence_topic, '{"status":"online"}', '{ retain: false }');
    }
  }
  var mqttData = {
  ...
  if (peripheral.advertisement.serviceData) {
    peripheral.advertisement.serviceData.forEach(function (d) {
      ...
      if (decoded !== d.data) {
        ...
        decoded.last = Math.floor(Date.now() / 1000);
        dev.filterAttributes(decoded);

        if (config.homeassistant) homeassistant.configDiscovery(decoded, dev, peripheral, d.uuid);
        if (config.homed) homeassistant.homedDiscovery(decoded, dev, peripheral, d.uuid);
        ...
        if (config.mqtt_format_json) {
          //avaks  mqtt.send(dev.json_state_topic + "/" + d.uuid, JSON.stringify(dev.getOrSetState(d.uuid, decoded)));
          mqtt.send(dev.json_state_topic, JSON.stringify(dev.getOrSetState(d.uuid, decoded)));
        }
      }
    });
  }
...
function checkForPresence() {
  ...
  Object.keys(inRange).forEach(function (addr) {
    ...
    if (inRange[addr].lastSeen < timeout) {
      if (config.retain) {
        mqtt.send(inRange[addr].dev.presence_topic, '{"status":"offline"}', '{ retain: true }');
      } else {
        mqtt.send(inRange[addr].dev.presence_topic, '{"status":"offline"}', '{ retain: false }');
      }
      delete inRange[addr];
    }
  });
...
exports.init = function () {
  ...
  noble.on("scanStart", function () {
    ...
    if (config.scanTimeout > 0 && config.scanInterval > 0) {
      // Остановить сканирование через n секунд
      stopScanTimeout = setTimeout(function () {
        exports.stopScan();

        // Запустить сканирование снова через n секунд
        scanInterval = setTimeout(function () {
          exports.startScan();
...
exports.sendMQTTPresence = function () {
  ...
  for (let addr in inRange) {
    if (config.retain) {
      mqtt.send(inRange[addr].dev.presence_topic, '{"status":"online"}', '{ retain: true }');
    } else {
      mqtt.send(inRange[addr].dev.presence_topic, '{"status":"online"}', '{ retain: false }');
    }
  }
  for (let mac in devices.list) {
    if (devices.list[mac].known) {
      if (config.retain) {
        mqtt.send(devices.list[mac].presence_topic, (devices.list[mac].mac in inRange) ? '{"status":"online"}' : '{"status":"offline"}', '{ retain: true }');
      } else {
        mqtt.send(devices.list[mac].presence_topic, (devices.list[mac].mac in inRange) ? '{"status":"online"}' : '{"status":"offline"}', '{ retain: false }');
      }
    }
  }
}
