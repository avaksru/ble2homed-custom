...
async function onDiscovery(peripheral) {
  ...
  if (entered) {
    ...
    if (config.retain == true) {
      ...
      mqtt.send(dev.presence_topic, '{"status":"online"}');
    }
  }
  var mqttData = {
  ...
  Object.keys(inRange).forEach(function (addr) {
    ...
    if (inRange[addr].lastSeen < timeout) {
      if (config.retain == true) {
        ...
        mqtt.send(inRange[addr].dev.presence_topic, '{"status":"offline"}');
      }
      delete inRange[addr];
    }
  });
...
exports.sendMQTTPresence = function () {
  ...
  for (let addr in inRange) {
    if (config.retain == true) {
      ...
      mqtt.send(inRange[addr].dev.presence_topic, '{"status":"online"}');
    }
  }
  for (let mac in devices.list) {
    if (devices.list[mac].known) {
      if (config.retain == true) {
        ...
        mqtt.send(devices.list[mac].presence_topic, (devices.list[mac].mac in inRange) ? '{"status":"online"}' : '{"status":"offline"}');
      }
    }
  }
}
